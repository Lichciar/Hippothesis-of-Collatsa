...
2. Для выявления производительности изменений скрипта добавить
   предельное значение натурального числа после которого рассчёт
   будет остановлен и будет выведено время работы скрипта и
   размер БД.
...
6. Для эксперимента попробывать удалённые БД, установленные на другом
   ПК. Например БД Firebird.
7. Сравнить скорость работы с программой на си.
8. При запуске проверять контрольную сумму последних 100 вычислений.
9. Разделить проект на клиента, который будет делать рассчёт. Сервер,
   который будет выделять объём натуральных чисел для вычисления. И
   клиенты тестировщаки, которые будет проверять обратным путём.
10. Проект наоборот:
    а. Берём 1 - это первая степеть.
    б. Второе натуральное число будет равно х * 2 - это вторая степень.
    в. Третье натуральное число будет равно (x - 1) / 3 - это
       вторая степеть. Если это число меньше 1 или равно 0, то
       откидываем его - это тупик.
    г. С числами второй степени проводи поочерёдно п. б-в - это
       третья степень. И т. д."""
